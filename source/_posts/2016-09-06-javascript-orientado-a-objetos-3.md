...
No Padrão Construtor com Protótipos, o objeto é criado por um construtor e os métodos são colocados no protótipo. Como a utilização de construtores com protótipos passou a ser uma técnica muito popular para a programação orientada a objetos em JavaScript, APIs nativas foram modificadas para tornar esse processo mais simples e menos verboso no código. Para criar um construtor com protótipos, utilizamos a propriedade `prototype` de objetos do tipo `Function`.

Vamos refatorar o código do nosso construtor `Person` para utilizar protótipos. As propriedades do objeto devem ser sempre do objeto e por isso continuam sendo atribuídas dentro do construtor. Os métodos `getAge` e `getFirstName` agora pertencerão ao protótipo dos objetos criados por `Person`. Veja no código a seguir:
...
Utilizando construtores com protótipos, os métodos `getAge` e `getFirstName` agora são único para todas as instâncias, uma vez que os métodos se encontram no protótipo das instâncias, que é único também. Note que a forma de criar os objetos permanece o mesmo e acessar seu protocolo é o mesmo.
...
O resultado é verdadeiro porque a referência do método de um objeto e do outro apontam para a mesma área de memória, ou seja, são o mesmo objeto. Algumas observações são importantes:
 ...
 - Caso você atribua um método chamado  `getAge` no objeto, ele não sobreescreverá o protótipo, mas adicionará esse método apenas para o objeto;
 - Note que há diferença entre o modo que um protótipo é associado com o construtor — por meio da propriedade `prototype` — e o modo que os objetos possuem protótipos — por meio da função `Object.getPrototypeOf`. Quando, por exemplo, `new Person(name, date)` é chamado, o protótipo do objeto retornado referencia o `Person.prototype`


---
## Herança

Com construtores com protótipos, a herança é sempre alcançada em duas etapas: transferência de propriedades e encadeamento de protótipos. A primeira parte, a transferência de propriedades, é feita da mesma forma que os construtores simples, utilizando o método `apply` de um construtor no contexto `this` atual. Dessa forma, todas as propriedades que o construtor chamado possuir serão atribuídos ao contexto `this` do objeto. Caso você queira entender melhor esse processo, veja como funciona herança [no texto anterior dessa série](http://maxroecker.github.io/blog/javascript-orientado-a-objetos-2/#herança).

A segunda parte consiste em encadear os protótipos e alinhar as referências aos construtores com o intuito de completar a cadeia de protótipos. Esse processo é bastante simples e feito por meio de uma função comumente chamada de `inherit`. Veja o código dela:
...
Observe que há a cópia de todas as propriedades de  `parents` e `prototype` para um novo objeto denominado `inheritance`. Esse objeto se tornará o protótipo dos objetos costruídos por `constructor`. Esse processo de cópia de propriedades de vários objetos para outro é comumente chamado de *mixin*.
...
Primeiramente, vamos ao nosso construtor `Building`. Seu código é semelhante ao da figura a seguir:
...
Agora, vamos fazer nosso construtor `Residential`, que herda diretamente as propriedades de `Building`. Além de utilizar o método `apply`, visto no capítulo anterior, criamos a cadeia de protótipos utilizando a função `Object.create`. A função `Object.create` é responsável por criar objetos com características específicas. De forma especial, seu primeiro parâmetro indica o objeto protótipo do objeto a ser criado. Veja no código a seguir:

{% simplecode js %}
``` js
var residentialParents = [ Building ],
    residentialPrototype = {
...
Residential.prototype = inherit(Residential,
                                residentialParents,
                                residentialPrototype);
```
{% endsimplecode %}

A delegação de atributos na cadeia de protótipos sempre começa sua “busca” pelo primeiro elemento. Por isso que quando executa-se uma função que pertence a um protótipo, mas que acessa atributos com a palavra chave `this`, é o atributo do objeto que será acessado — caso ele não exista, a delegação continua a subir na cadeia de protótipos.
...
WorkingBuilding.prototype = inherit(WorkingBuilding,
                                    workingBuildingParents,
                                    workingBuildingPrototype );
```
{% endsimplecode %}
...
### Herança Múltipla
A herança múltipla é possível em JavaScript quando utilizamos Construtores com Protótipos. Primeiro é necessário executar o `apply` de cada um dos construtores dos quais se quer herdar propriedades no construtor. Após isso, basta adicioná-lo no arranjo `parents` e passá-lo para a função `inherit`.

Para exemplificarmos, vamos codificar nosso construtor `Condominium`, que herda propriedades de  `Complex` e de `Residential`:
...
Por hoje é só, pessoal. No próximo post da série vamos compreender o Padrão Fábrica para a criação de objetos, que vem para resolver alguns problemas que ambos os padrões baseados em construtores tem: falhas que podem ocorrer devido ao uso da palavra-chave `new`.
