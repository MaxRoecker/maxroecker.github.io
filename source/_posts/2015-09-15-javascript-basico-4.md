...
**Importante:** Em JavaScript, um bloco não define o escopo das variáveis declaradas com `var`, como é o caso de linguagens como o C ou Java. O escopo em JavaScript está intimamente ligado com a declaração de funções ─ vamos aprofundar o assunto de escopo de variáveis em artigos futuros. Veja o exemplo:
...
A declaração `if` tem o comportamento de executar a próxima declaração ou bloco de declarações, se a condição avaliada seja `true`. Pode também ser utilizada com o `else` e assim executar a declaração que o sucede se o valor da condição for `false`. A sintaxe do `if` pode ser dada:
...
Nas declarações condicionais, a condição precisa ser uma expressão que seja avaliada em `true` ou `false`. Mas não necessariamente precisa ser do tipo booleano que já vimos anteriormente. O JavaScript, em sua conversão de tipos, avalia alguns valores especiais automaticamente para `false` quando passados em uma condição. Esses valores são chamados de ***Falsy***. São eles:

* `undefine` e `null`
* Valor numérico `0` e `NaN`
* Uma String vazia `""`

Qualquer outro valor, incluindo objetos, são avaliados como `true`. E por isso é preciso atenção especial com valores primitivos booleanos `true` e `false` com os valores booleanos do objeto Boolean. Veja o exemplo abaixo:
...
Para cada tipo de dado da condição, o `switch` apresenta um comportamento diferente:
...
Vamos ver na prática! Teste o código abaixo, alterando o valor da variável `temperatura` e `clima` para ver outros resultados.
...
{% endsimplecode %}
Entendeu o comportamento do código? O JavaScript, ao se deparar com uma declaração `switch`, primeiramente avalia a expressão de condição e inicia uma cadeia de busca de rótulos que combinem com a condição. Caso houver combinação, as declarações associadas a ele são executadas. Quando todas as declarações foram executadas, então o JavaScript **procura pelo próximo rótulo que combine com valor e repete o processo**. Quando o rótulo `default` é atingido, as declarações associadas a ele são executadas automaticamente, independente do valor.

Caso você queira que somente um dos rótulos seja combinado com a condição a expressão `break` ─ saída do bloco ─ ao fim das declarações de cada rótulo. Dessa forma, o primeiro rótulo combinado é executado e sai fora do `switch`.
...
Por conveção, o rótulo `default` é sempre colocado como único ao final do bloco, mas não precisa necessariamente ser assim. ***:)***
...
Se a condição for eventualmente `falsa`, o laço para de ser executado e o controle para a próxima declaração após ele. A condição é sempre testada **antes** das declarações serem executadas.

Vamos calcular 2<sup>10</sup> utilizando o `while`, veja o código abaixo:
...
### do .. while
A declaração `do ... while`,  tem um comportamento parecido com o `while`: executa a declaração enquanto a condição for verdadeira. Sua sintaxe é:
...
A diferença entre as duas é que a condição é testada **depois** da execução da declaração. Se a condição for `false`, o laço para de ser executado e o controle para a pŕoxima declaração após ele.

Usando o mesmo exemplo anterior, vamos calcular 2<sup>10</sup> utilizando o `do...while`, veja o código abaixo:
...
Um laço `for` também repete a próxima declaração até que a condição dada seja `false`. Porém, junto com a condição, ela carrega outras duas declarações: a inicialização e o passo. Tem sintaxe parecida com Java e C:
...
Um `for`, ao ser executado, executa os seguintes passos ordenadamente:
...
Vamos ver o mesmo exemplo de cálculo de 2<sup>10</sup> escrito com `for`:

{% simplecode js %}
``` js
var resultado = 1;

for ( var expoente = 0; i < 10; expoente++ )
  resultado = resultado * 2;
...
#### break
A declaração `break` sem um rótulo interrompe a execução de um laço ou `switch`, como visto anteriormente, mais interno. Ou seja, se há dois laços aninhados e houver um break, o laço "de dentro" será interrompido.

Quando utilizados com um `label`, interrompe a execução da declaração rotulada com o `label`. Sua sintaxe é simples:
...
#### continue
A declaração `continue` tem um comportamento diferente quando acompanhado de um rótulo ou não. Quando sem rótulo, o `continue` pula a atual iteração e passa para a próxima do laço mais interno. Quando acompanhado de um rótulo, faz o mesmo processo para o laço rotulado.

Ao contrário do `break` que cancela a execução do laço, o `continue` somente pula a iteração atual. Caso seja um `while` ou  `do ... while`, o `continue` pula para a verificação da condição. Caso seja um `for`, ele pula para a declaração de passo. Sua sintaxe é:
...
## Declaração vazia
Uma declaração vazia é somente um `;`, indicando que nenhuma outra declaração deve ser executada. Uma declaração vazia pode ser utilizada em algumas declarações de laço. Veja o exemplo abaixo onde zeramos um array com um `for` em declaração um vazia:
