...
propriedade `email` sequer é verificada. Ou seja, esse algoritmo é linearmente
proporcional a quantidade de propriedades que temos que comparar.
...
propriedade à propriedade pode demorar um pouco…
...
distribuição uniforme. Assim, nenhum intervalo terá mais ou menos chances sofrer
colisão.
...
<p>
  Falsos positivos podem acontecer, mas falsos negativos não. Ou seja, a
...
A CRUXHash é uma biblioteca provê um conjunto de funções simples para criar
...
seu resultado para o `hash`. Esse comportamento é util para reutilizar objetos
...
Finaliza por aqui a apresentação da CRUXHash. A biblioteca disponibiliza outras
funções para usos em casos mais específicos. Por isso, recomendo que você dê uma
olhada no [repositório oficial](https://github.com/MaxRoecker/cruxhash) para ver
a documentação da API disponível.

Criei essa biblioteca de acordo com minha necessidade em projetos pessoais e
profissionais. Caso ela não seja adequada para você e você queira adicionar
algum comportamento ainda não disponível, entre em contato comigo. PRs são
sempre bem vindos!
